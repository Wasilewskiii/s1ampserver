# Schedule I Dedicated Server — AMP Template

An [AMP (CubeCoders)](https://cubecoders.com/AMP) deployment template that runs a **Schedule I dedicated server** using the [S1DedicatedServers mod](https://github.com/ifBars/S1DedicatedServers) by ifBars. It wraps the official S1DS Docker image (`ghcr.io/ifbars/s1dedicatedservers`), which ships the whole Wine + Xvfb + MelonLoader + SteamCMD stack — so AMP gives you start/stop, console, player list, scheduling and backups, while upstream maintains the tricky parts.
...
4. **Start** the server and watch the console:
   ...
   - First boot downloads ~7 GB via SteamCMD and (on IL2CPP) runs a one-time Cpp2IL code generation — expect 15–30 minutes. Later starts are fast.
5. After first boot, edit `s1ds/game/UserData/server_config.toml` (AMP file manager):
   - `serverName`, `maxPlayers`, and set a `serverPassword` unless you want the server open
   - `steamGameServerQueryPort = 27018` (must match the template's Steam Query Port setting)
...
- World data and config live in `s1ds/game/UserData/` — that's the part worth backing up. AMP's Backups tab works normally.
- To import an existing world, follow the [official save-path guide](https://github.com/ifBars/S1DedicatedServers/blob/master/docfx/docs/configuration/save-path.md) — never overwrite the server's `Players/Player_0` data.
